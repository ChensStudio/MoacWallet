...
updateMistBadge = function(){
    ...
    if(typeof mist !== 'undefined' && (!conf || !conf.confirmedOwners.length)) {
        ...
        mist.menu.setBadge(McTools.formatBalance(balance, '0.0 a','mc') + ' MC');
    }
};
...
updateMistMenu = function(){
    ...
    var accounts = _.union(Wallets.find({}, {sort: {name: 1}}).fetch(), McAccounts.find({}, {sort: {name: 1}}).fetch());
    ...
    Meteor.setTimeout(function(){
        ...
        _.each(accounts, function(account, index){
            mist.menu.add(account._id,{
                position: 3 + index,
                name: account.name,
                badge: McTools.formatBalance(account.balance, "0 a", 'mc')+ ' MC',
