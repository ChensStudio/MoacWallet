...
chain3.mc.isSyncing(function(error, syncing) {
    if(!error) {
...
var connect = function(){
    ...
    if(chain3.isConnected()) {
        ...
        chain3.mc.getSyncing(function(e, sync) {
            if(e || !sync) {
                connectToNode();
            } else {
                McAccounts.init();
        ...
        Meteor.setTimeout(function(){
            // if in mist, tell to start moac, otherwise start with RPC
            var mcRPC = (chain3.admin) ? 'moac' : 'moac --rpc --rpccorsdomain "'+window.location.protocol + '//' + window.location.host+'"';

            McElements.Modal.question({
                text: new Spacebars.SafeString(TAPi18n.__('wallet.app.texts.connectionError' + (chain3.admin ? 'Mist' : 'Browser'), 
                    {node: mcRPC})),
                ok: function(){
