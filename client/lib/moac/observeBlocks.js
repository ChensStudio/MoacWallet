...
var getPeerCount = function() {
    chain3.net.getPeerCount(function(e, res) {
        if(!e)
...
updateBalances = function() {
    ...
    _.each(creatingWallets, function(wallet){
      ...
      chain3.mc.getTransactionReceipt(wallet.transactionHash, function(error, receipt) {
    ...
    // UPDATE ENS
    var allAccounts = McAccounts.find().fetch().concat(walletsAndContracts);
        ...
        if (!account.ensCheck || (account.ensCheck && now - account.ensCheck > 10*60*1000)) {
            Helpers.getENSName(account.address, function(err, name, returnedAddr) {
                ...
                if (!err && account.address.toLowerCase() == returnedAddr){
                    McAccounts.update({address: account.address}, {$set:{ name: name, ens: true, ensCheck: now}});
                    ...
                    McAccounts.update({address: account.address}, {$set:{ens: false, ensCheck: now}});
    ...
    var walletsContractsAndAccounts = McAccounts.find().fetch().concat(Wallets.find().fetch());
...
observeLatestBlocks = function(){
    ...
    chain3.mc.filter('latest').watch(function(e, res){
        if(!e) {
            updateBalances();
        }
    });
