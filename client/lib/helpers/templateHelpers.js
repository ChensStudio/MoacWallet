...
/**
Check if currency unit is an mc unit
...
Template.registerHelper('selectAccounts', function(hideWallets){
    var accounts = McAccounts.find({balance:{$ne:"0"}}, {sort: {balance: 1}}).fetch();

    if(hideWallets !== true)
        accounts = _.union(Wallets.find({owners: {$in: _.pluck(McAccounts.find().fetch(), 'address')}, address: {$exists: true}}, {sort: {name: 1}}).fetch(), accounts);
...
Formats a given transactions balance
    ...
    {{formatTransactionBalance value exchangeRates "mc"}}
...
Template.registerHelper('toChecksumAddress', function(address){
    return _.isString(address) ? chain3.toChecksumAddress(address) : '';
});
