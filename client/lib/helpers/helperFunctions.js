...
import '../../templates/elements/inputs/address.js';
import '../../templates/elements/inputs/bool.js';
import '../../templates/elements/inputs/bytes.js';
import '../../templates/elements/inputs/int.js';
import '../../templates/elements/inputs/json.js';
import '../../templates/elements/inputs/string.js';
import '../../templates/elements/inputs/uint.js';
...
Helpers.getDefaultContractExample = function(withoutPragma) {
    ...
    if (withoutPragma) {
        ...
        // so that older versions that will query code from wallet.moac.org won't use broken example code.
...
Helpers.getOwnedAccountFrom = function(accountList){
    ...
    var accounts = McAccounts.find({}, {sort: {balance: 1}}).fetch();
...
Helpers.formatNumberByDecimals = function(number, decimals){
    ...
    return McTools.formatNumber(new BigNumber(number, 10).dividedBy(Math.pow(10, decimals)), numberFormat);
};
...
Helpers.checkChain = function(callback){
    ...
    chain3.mc.getCode(originalContractAddress, function(e, code){
...
Helpers.isWatchOnly = function(id) {
    return !Wallets.findOne({_id: id, owners: {$in: _.pluck(McAccounts.find({}).fetch(), 'address')}});
};
...
/**
Gets the docuement matching the given addess from the McAccounts or Wallets collection.
...
/**
Gets the docuement matching the given query from the McAccounts or Wallets collection.
...
Helpers.getAccounts = function(query, reactive) {
    ...
    return McAccounts.find(query, options).fetch().concat(Wallets.find(query, options).fetch());
};

/**
Gets the docuement matching the given addess from the McAccounts or Wallets collection and returns its name or address.
...
var sha3 = function(str, opt) {
  return '0x' + chain3.sha3(str, opt).replace('0x','');
};
...
Helpers.getENSName = function(address, callback) {
    ...
    var ensContract = chain3.mc.contract(ensContractAbi);
    var resolverContract = chain3.mc.contract(resolverContractAbi);
