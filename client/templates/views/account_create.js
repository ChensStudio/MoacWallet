...
Template['views_account_create'].helpers({
    ...
    */
    'defaultDailyLimit': function() {
        ...
        return typeof dailyLimit != 'undefined' ? chain3.fromSha(dailyLimit,'mc') :  10;
    },
    /**
    Default Name
    ...
    'submit': function(e, template){
        ...
        if(type === 'simple') {
            Wallets.insert({
                ...
                balance: '0',
                creationBlock: McBlocks.latest.number,
                code: code
            });
        ...
        if(type === 'multisig') {
            ...
            var owners = _.uniq(_.compact(_.map(template.findAll('input.owners'), function(item){
                if(chain3.isAddress(item.value))
            ...
            Wallets.insert({
                ...
                name: template.find('input[name="accountName"]').value || TAPi18n.__('wallet.accounts.defaultName'),
                balance: '0',
                dailyLimit: chain3.toSha(formValues.dailyLimitAmount, 'mc'),
