...
import '../elements/compileContract.js';
import '../elements/inputs/address.js';
import './send.html';
            ...
            var sendTransaction = function (estimatedGas) {
                ...
                if (contracts['ct_' + selectedAccount._id]) {
                    ...
                    var isMicroChain = false;
                       /**
deploy contract to mainchain or subchain

@method sendTX
*/
            var sendTX = function (tranData) {
                chain3.mc.sendTransaction(tranData, function (error, txHash) {
                    ...
                    if (!error) {
                        ...
                        if (isMicroChain === false) {
                            ...
                            var thisMicroChain = MicroChainContracts.findOne({ address: to });
                            if (thisMicroChain) {
                                MicroChainContracts.update({ address: to }, {
                                    $push: {
                                        Dapps: {
                                            dappName: (contractName || 'New Contract') + ' ' + to.substr(2, 4),
                                            dappAddr: "",
                                            dappInterface: jsonInterface,
                                            TxHash: txHash
                                        }
                                    }
                                })
                                ...
                                MicroChainContracts.upsert({ address: to }, {
                                    $set: {
                                        address: to,
                    ...
                    if (microChainDapp) {
                        ...
                        chain3.scs.getNonce(to, selectedAccount.address, function (e, r) {
                            if (!e) {
                                tranData = {
